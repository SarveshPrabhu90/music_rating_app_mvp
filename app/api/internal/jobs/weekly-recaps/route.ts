...
import { generateWeeklyRecapsJob } from "@/lib/jobs/weekly-recaps";
...
export async function POST(request: Request) {
  const trace = createRequestTrace(request, "jobs.weekly_recap");
  ...
  const results = await generateWeeklyRecapsJob(parsed.data);
